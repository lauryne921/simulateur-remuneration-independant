...
function disambiguateReference(rules, contextName = "", partialName) {
  const possibleDottedName = [contextName, ...ruleParents(contextName), ""].map((x) => x ? x + " . " + partialName : partialName).sort(
    (a, b) => a === contextName ? 1 : b === contextName ? -1 : 0
  );
  const existingDottedName = possibleDottedName.filter((name) => name in rules);
  const accessibleDottedName = existingDottedName.find(
    (name) => isAccessible(rules, contextName, name)
  );
...
V\xE9rifiez que l'orthographe et l'espace de nom sont corrects`,
  ...
  if (!accessibleDottedName) {
    throw new PublicodesError(
      "SyntaxError",
      `La r\xE8gle "${existingDottedName[0]}" n'est pas accessible depuis "${contextName}".
...
function parseRule(nom, rawRule, context) {
  ...
  const explanation = {
    ...
    parents: ruleParents(dottedName).map((parent) => parse(parent, context)).map(
...
var evaluateContexte = function(node) {
  ...
  if (Object.keys(amendedSituation).length) {
    ...
    Object.values(amendedSituation).forEach(
      (value) => engine.cache.nodes.set(value, this.evaluate(value))
    );
...
var evaluate5 = function(node) {
  ...
  const to = this.evaluateNode(node.explanation["jusqu'\xE0"]);
  let nodeValue;
  if ([from, to].some(({ nodeValue: nodeValue2 }) => nodeValue2 === void 0)) {
    nodeValue = void 0;
  } else {
    ...
    nodeValue = Math.max(
      0,
      Math.round(
        (toDate.getTime() - fromDate.getTime()) / (1e3 * 60 * 60 * 24)
      )
    );
  }
  return {
...
var evaluateIsNotApplicable = function(node) {
  ...
  if (this.context.nodesTypes.get(valeur)?.isNullable === false && valeur.nodeKind !== "rule") {
...
var evaluateInversion = function(node) {
  ...
  (lastEvaluation.traversedVariables ?? []).forEach(
    (v) => this.cache._meta.traversedVariablesStack[0].add(v)
  );
  return {
    ...node,
    nodeValue,
...
var evaluate8 = function(node) {
  ...
  ) ? operatorFunction(convertToDate(a), convertToDate(b)) : operatorFunction(a, b);
...
var flattenJoin = ([a, b]) => Array.isArray(b) ? join([a, ...b]) : a;
var Lexer = lexer;
var ParserRules = [
  ...
  { "name": "Words$ebnf$1", "symbols": ["Words$ebnf$1", "Words$ebnf$1$subexpression$2"], "postprocess": function arrpush2(d) {
    return d[0].concat([d[1]]);
  } },
  ...
  { "name": "UnitDenominator$ebnf$1", "symbols": [], "postprocess": function(d) {
    return null;
  } },
  ...
  { "name": "UnitNumerator$ebnf$1", "symbols": [], "postprocess": function(d) {
    return null;
  } },
  ...
  { "name": "Unit$ebnf$1", "symbols": [], "postprocess": function(d) {
    return null;
  } },
  { "name": "Unit$ebnf$2", "symbols": [] },
  { "name": "Unit$ebnf$2", "symbols": ["Unit$ebnf$2", "UnitDenominator"], "postprocess": function arrpush3(d) {
    return d[0].concat([d[1]]);
  } },
...
function parseReference(v, context) {
  if (!context.dottedName) {
    throw new PublicodesInternalError({
      message: "Une r\xE9f\xE9rence ne peut pas exister en dehors d'une r\xE8gle (`context.dottedName` est vide)",
      context
    });
  }
  return {
...
function makeReplacementInliner(replacements, referencesMaps, logger) {
...
function replace(node, replacements, logger) {
...
function parsePublicodes(rawRules, partialContext = createContext({})) {
  ...
  let rules = weakCopyObj(rawRules);
...
function disambiguateReferencesAndCollectDependencies(parsedRules, newRules, referencesMaps) {
...
var Engine = class _Engine {
  ...
  evaluateNode(parsedNode) {
    ...
    const isTraversedVariablesBoundary = this.cache._meta.traversedVariablesStack.length === 0 || parsedNode.nodeKind === "rule";
    if (isTraversedVariablesBoundary) {
      this.cache._meta.traversedVariablesStack.unshift(/* @__PURE__ */ new Set());
    }
    if (parsedNode.nodeKind === "reference" && parsedNode.dottedName && parsedNode.dottedName in this.publicParsedRules) {
