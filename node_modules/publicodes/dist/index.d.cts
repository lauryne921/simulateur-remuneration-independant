...
type RendNonApplicable = Exclude<Remplace, {
    par: any;
}>;
...
type NodeKind = ASTNode['nodeKind'];
type TraverseFunction<Kind extends NodeKind> = (fn: ASTTransformer, node: ASTNode & {
    nodeKind: Kind;
}) => ASTNode & {
    nodeKind: Kind;
};
...
type EvaluatedNode<T extends Types = Types> = ASTNode & EvaluationDecoration<T>;
...
declare function disambiguateReference<R extends Record<string, RuleNode>>(rules: R, contextName: string | undefined, partialName: string): keyof R;
...
type EvaluationOptions = Partial<{
    unit: string;
}>;
